...
NSString* kWPAttributedSendUrlFromeText = @"WPAttributedSendUrlFromeText";
...
-(NSAttributedString*)attributedWithStyleBook{
    ...
    for (NSTextCheckingResult *match in arrayOfAllMatches)
    {
        ...
        NSDictionary* style3 = @{@"body":[UIFont fontWithName:@"HelveticaNeue" size:20.0],
                                 substringForMatch:[WPAttributedStyleAction styledActionWithAction:^{
                                     NSLog(@"%@",substringForMatch);
                                     [[NSNotificationCenter defaultCenter] postNotificationName:kWPAttributedSendUrlFromeText object:substringForMatch];
                                 }],
                                 @"link": [UIColor orangeColor]};
        
        NSObject* bodyStyle = style3[@"body"];
        if (bodyStyle) {
            [self styleAttributedString:as range:NSMakeRange(0, as.length) withStyle:bodyStyle withStyleBook:style3];
        }
        
        NSObject* style = style3[substringForMatch];
        
        [self styleAttributedString:as range:match.range withStyle:style withStyleBook:style3];
    }
